...
2026-05-27 更新：AI 智能录入页新增 TOML 批量导入入口；TOML 文本解析由前端完成，`.toml` 文件解析通过 `toml_select_file` / `toml_parse_file` Tauri 命令完成。当前已覆盖前端 Vitest、Rust `toml_tools` 目标测试、TypeScript 检查、lint 与 Vite 构建；真实 TOML 文件选择对话框仍归入文件选择人工验收范围。
