use crate::database::{
    CreateQuestionInput, DatabaseStore, ImportError, ImportResult, QuestionRepository,
};
use crate::error::AppError;

/// 题目导入业务服务。
///
/// 统一收口 CSV、JSON、AI 解析结果的批量导入业务编排，通过 [`QuestionRepository`] 写入数据库。
///
/// ## 层次结构
/// `CsvCommand / QuestionCommand / AiCommand` → `ImportService` → `QuestionRepository` → `DatabaseStore` → SQLite
...
impl ImportService {
    ...
    ///
    /// ## 业务流程
    /// 1. 基础参数校验（bank_id > 0、questions 非空）由 `QuestionRepository::create_batch` 负责
    /// 2. 逐题字段校验（`validate_question`），收集 `ImportError`
    /// 3. 有效题目在单个事务中批量写入，失败则整体回滚
    /// 4. 返回 `ImportResult` 包含 success / failed / errors
    ///
    /// ## 复用的 Repository 方法
    /// - `QuestionRepository::create_batch`：委托 `create_questions_batch`（含校验 + 事务 + operation_log）
    pub fn import_questions(
        &self,
        bank_id: i64,
        questions: Vec<CreateQuestionInput>,
    ) -> Result<ImportResult, AppError> {
        Ok(self.repo.create_batch(bank_id, questions)?)
    }
