        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 max-w-4xl text-xs leading-5 text-gray-500 dark:text-gray-400">
              当当前 Tauri 数据库已有数据时，旧 Electron
              数据库不会被自动覆盖；如需切换旧库，必须先备份当前库再显式替换。
            </p>
          </div>
        </div>
        <ActionButton
      ...
      <div className="space-y-4">
        ...
        {migrationResult && (
          <AlertBanner type="success" title="已备份并使用旧库替换">
            当前数据库已替换；备份路径：{migrationResult.backupPath || '无旧库备份'}
            。请重启应用确认数据。
          </AlertBanner>
        )}
          ...
          <>
            {needsExplicitReset ? (
              <AlertBanner type="warning" title="检测到旧库数据">
                当前 Tauri
                数据库和旧数据库都包含用户数据，系统不会自动覆盖。请确认后选择一个旧库执行备份替换。
              </AlertBanner>
            ) : (
              <AlertBanner
                type={migrationStatus.recommendedAction === 'auto_migrate' ? 'info' : 'success'}
              >
              ...
              {legacyCandidatesWithData.length === 0 ? (
                ...
                legacyCandidatesWithData.map((candidate) => {
                  ...
                  return (
                    <div
                      key={candidate.path}
                      className="rounded-2xl border border-gray-100 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/70"
                    >
