...
export function QuestionBankCard({
  ...
  onDelete,
  formatDate,
  toneClass: _toneClass = 'bg-blue-50 text-primary',
}: {
  bank: {
    id: number;
    name: string;
  ...
  return (
    <SurfaceCard
      hover
      as="article"
      className="group cursor-pointer overflow-hidden hover:shadow-none [&:hover]:border-transparent"
      padding="p-0"
      onClick={onClick}
    >
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1 py-4 pr-4">
          <h3 className="line-clamp-2 text-sm font-extrabold leading-5 text-gray-900 dark:text-white">
            {bank.name}
          </h3>
      ...
      <p className="mt-3 line-clamp-2 min-h-[36px] px-5 text-xs leading-5 text-gray-500 dark:text-gray-400">
...
export function PracticeCard({
  ...
  bank: { id: number; name: string; questionCount?: number; question_count?: number };
  icon?: ElementType;
  index?: number;
  selected?: boolean;
  onSelect?: () => void;
  onStart?: ButtonHTMLAttributes<HTMLButtonElement>['onClick'];
}) {
  const iconSrc = getRandomIcon(bank.id);
  ...
  return (
    <SurfaceCard
      ...
      as="article"
      className={cn(
        'group cursor-pointer overflow-hidden hover:shadow-none [&:hover]:border-transparent',
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1 py-4 pr-4">
          <h3 className="line-clamp-2 text-sm font-extrabold text-gray-900 dark:text-white">
            {bank.name}
          </h3>
...
export function AnswerOptionCard({
  ...
  const stateClass = {
    default:
      'border-gray-200 bg-white hover:border-primary hover:bg-blue-50/60 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700',
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'w-full rounded-2xl border p-4 text-left transition-all active:scale-[0.99] disabled:cursor-default',
    ...
    <SurfaceCard className="mx-auto max-w-2xl text-center" padding="p-8">
      ...
      <div className="my-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {stats.map((item) => (
          <div
            key={item.label}
            className={cn('rounded-2xl p-4', item.className || 'bg-gray-50 dark:bg-gray-700')}
          >
